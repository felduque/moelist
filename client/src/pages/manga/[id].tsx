...
const CardManga: NextPage<ContentType | undefined> = (content) => {
  return (
    <>
      <Head>
        ...
        <meta name="author" content={content?.User?.userName as string} />
        ...
        <div className={`row pt-5 ${style.content_sinopsis_and_banner}`}>
          <div className="col-12 col-xl-3 text-center">
            ...
            <AuthorBox
              ...
              author={(content?.User?.userName as string) || ""}
            />
          </div>
...
CardManga.getInitialProps = async (ctx: NextPageContext) => {
  const { id } = ctx.query;
